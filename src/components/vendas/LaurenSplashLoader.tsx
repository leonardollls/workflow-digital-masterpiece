...
interface LaurenSplashLoaderProps {
  onComplete: () => void;
  duration?: number;
}
...
const generateParticleStyles = (count: number): ParticleStyle[] => {
  ...
  for (let i = 0; i < count; i++) {
    // Usar valores pseudorandom baseados no índice para consistência
    const seed = i * 17 + 7;
    const width = ((seed % 30) / 10) + 1; // 1-4px
    const height = width;
    const left = (seed * 7 % 100); // 0-100%
    const top = (seed * 13 % 100); // 0-100%
    const r = 212 + (seed % 20);
...
/**
 * Splash Loader customizado para Lauren Rossarola - Odontologia
 * - Exibe a marca "LR" (Lauren Rossarola)
 * - Cores elegantes alinhadas com a identidade visual (tons dourados/bege)
 */
const LaurenSplashLoader = ({ onComplete, duration = 1200 }: LaurenSplashLoaderProps) => {
  ...
  const words = ['Excelência', 'Confiança', 'Naturalidade', 'Sorrisos'];
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 text-center transition-all duration-500 ${
        ...
        <div className="relative mb-6" style={{ width: '140px', height: '140px', margin: '0 auto', contain: 'layout' }}>
          ...
          <svg className="w-full h-full mx-auto transform -rotate-90" viewBox="0 0 100 100">
            ...
            <circle
              ...
              r="45"
              fill="none"
              stroke="url(#laurenProgressGradient)"
              strokeWidth="3"
              strokeLinecap="round"
            ...
            />
            <defs>
              <linearGradient id="laurenProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#122737] to-[#D4A574] flex items-center justify-center shadow-lg logo-pulse">
              <span className="text-white font-bold text-2xl tracking-tight">LR</span>
            </div>
          </div>
        </div>
        ...
        {/* Subtitle */}
        <p className="text-sm text-white/50 mb-4 tracking-widest uppercase">
...
export default LaurenSplashLoader;
