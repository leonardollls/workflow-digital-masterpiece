...
interface RevitalleSplashLoaderProps {
  onComplete: () => void;
  duration?: number;
}
...
const RevitalleSplashLoader = ({ onComplete, duration = 1200 }: RevitalleSplashLoaderProps) => {
  ...
  const words = ['Implantes', 'Qualidade', 'Confiança', 'Sorriso'];
        ...
        <div className="relative mb-6" style={{ width: '140px', height: '140px', margin: '0 auto', contain: 'layout' }}>
          ...
          <svg className="w-full h-full mx-auto transform -rotate-90" viewBox="0 0 100 100">
            ...
            <circle
              ...
              r="45"
              fill="none"
              stroke="url(#revitalleProgressGradient)"
              strokeWidth="3"
              strokeLinecap="round"
            ...
            />
            <defs>
              <linearGradient id="revitalleProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#122737] to-[#D4A574] flex items-center justify-center shadow-lg logo-pulse">
              <span className="text-white font-bold text-3xl">RV</span>
            </div>
          </div>
        </div>
...
export default RevitalleSplashLoader;
