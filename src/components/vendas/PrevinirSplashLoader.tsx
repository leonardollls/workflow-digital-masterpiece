...
interface PrevinirSplashLoaderProps {
  onComplete: () => void;
  duration?: number;
}
...
const generateParticleStyles = (count: number): ParticleStyle[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Cores alinhadas com Lidderà (dourado/bronze)
    const r = 212 + (seed % 20);
...
/**
 * Splash Loader customizado para Prevenir Odontopediatria
 * - Exibe a marca "Prevenir" 
 * - Cores alinhadas com a identidade visual da Lidderà (dourado/bronze)
 */
const PrevinirSplashLoader = ({ onComplete, duration = 1200 }: PrevinirSplashLoaderProps) => {
  ...
  const words = ['Cuidado', 'Prevenção', 'Sorriso', 'Confiança'];
        ...
        <div className="relative mb-6" style={{ width: '140px', height: '140px', margin: '0 auto', contain: 'layout' }}>
          ...
          <svg className="w-full h-full mx-auto transform -rotate-90" viewBox="0 0 100 100">
            ...
            <circle
              ...
              r="45"
              fill="none"
              stroke="url(#previnirProgressGradient)"
              strokeWidth="3"
              strokeLinecap="round"
            ...
            />
            <defs>
              <linearGradient id="previnirProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
...
export default PrevinirSplashLoader;
